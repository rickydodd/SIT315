...
int main()
{
    ...
    // measure time at beginning of parallelised multiplication, C = AB.
    auto start = steady_clock::now();
    
    #pragma omp parallel for private(i, j, k) shared(d) // compiler directive to parallelise block
    for(i = 0; i < MAX; i++)
        for(j = 0; j < MAX; j++)
            for(k = 0; k < MAX; k++)
    ...
    // measure time at end of parallelised multiplication
